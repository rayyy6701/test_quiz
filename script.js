...
const createSession = (random = Math.random) => ({
  version: 1,
  attemptId: `attempt-${Date.now()}-${Math.floor(random() * 1000000)}`,
  screen: 'quiz',
  currentIndex: 0,
...
const calculateExamScore = (session) => {
  const score = session.items.filter((item) => {
    if (!item.primaryCorrect) return false
    const question = questions.find((entry) => entry.id === item.questionId)
    return !question?.followUp || item.followUpCorrect === true
  }).length
  return { score, passed: score >= 40 }
}

const validScoreEntry = (entry) => entry
  && typeof entry.id === 'string' && entry.id.length > 0
  && typeof entry.completedAt === 'string' && !Number.isNaN(Date.parse(entry.completedAt))
  && Number.isInteger(entry.score) && entry.score >= 0 && entry.score <= 60
  && typeof entry.passed === 'boolean' && entry.passed === (entry.score >= 40)

const validateScoreHistory = (value) => Array.isArray(value) ? value.filter(validScoreEntry) : []

const appendScoreHistory = (history, entry) => {
  const valid = validateScoreHistory(history)
  if (!validScoreEntry(entry) || valid.some((item) => item.id === entry.id)) return valid
  return [entry, ...valid]
}
...
const SCORE_HISTORY_KEY = 'quiz-710-score-history-v1'
...
const loadScoreHistory = () => {
  try {
    return validateScoreHistory(JSON.parse(localStorage.getItem(SCORE_HISTORY_KEY)))
  } catch {
    return []
  }
}

const saveCompletedScore = (session) => {
  const result = calculateExamScore(session)
  const entry = {
    id: session.attemptId || `attempt-${Date.now()}`,
    completedAt: new Date().toISOString(),
    score: result.score,
    passed: result.passed,
  }
  scoreHistory = appendScoreHistory(scoreHistory, entry)
...
const scoreHistoryHtml = () => `<section class="card score-history">
  <h2>スコア履歴</h2>
  ${scoreHistory.length ? `<ol>${scoreHistory.map((entry) => `<li>
    <time datetime="${entry.completedAt}">${escapeHtml(new Date(entry.completedAt).toLocaleString('ja-JP'))}</time>
    <strong>${entry.score} / 60点</strong>
    <span class="${entry.passed ? 'pass' : 'fail'}">${entry.passed ? '合格' : '不合格'}</span>
  </li>`).join('')}</ol><button class="secondary clear-scores" type="button">スコア履歴を消す</button>` : '<p>まだ保存されたスコアはありません。</p>'}
</section>`

const bindScoreHistory = () => document.querySelector('.clear-scores')?.addEventListener('click', () => {
  if (!confirm('保存したスコア履歴をすべて消しますか？')) return
...
const renderResult = () => {
  ...
  const exam = calculateExamScore(activeSession)
  ...
  appRoot.innerHTML = `<main class="shell">
    <section class="card result-card">
      <span class="eyebrow">学習完了</span><h1>${exam.score} / 60点</h1>
      <p class="judgement ${exam.passed ? 'pass' : 'fail'}">${exam.passed ? '合格' : '不合格'}</p>
...
const renderQuiz = () => {
  ...
  document.getElementById('next')?.addEventListener('click', () => {
    const next = advance(activeSession)
    if (activeSession.screen !== 'result' && next.screen === 'result') saveCompletedScore(next)
    update(next)
  })
}
...
window.QuizApp = { questions, shuffle, createSession, answerPrimary, answerFollowUp, advance, calculateResult, calculateExamScore, validateSavedSession, validateScoreHistory, appendScoreHistory }
render()
